...
INNER JOIN 
    inventory AS i ON i.inventory_id = r.inventory_id -- Join inventory to link rentals to films
INNER JOIN 
    film_category AS fc ON i.film_id = fc.film_id -- Join film_category to get film details
INNER JOIN 
    category AS cat ON cat.category_id = fc.category_id -- Join category to filter by film category
INNER JOIN 
...
WITH RevenueGenerated AS (
    ...
    INNER JOIN 
        inventory AS i ON i.inventory_id = r.inventory_id -- Join inventory
    INNER JOIN 
        film_category AS fc ON i.film_id = fc.film_id -- Join film_category
    INNER JOIN 
        category AS cat ON cat.category_id = fc.category_id -- Join category
    INNER JOIN 
        store AS st ON s.store_id = st.store_id -- Join store
    WHERE 
        EXTRACT(YEAR FROM pay.payment_date) = 2017  -- Filter for payments in 2017
    GROUP BY 
        s.staff_id, s.first_name, s.last_name, st.store_id -- Group results
),
RankedEmployees AS (
    SELECT 
        staff_id,
        ...
        store_id,
        total_revenue,
        ROW_NUMBER() OVER (ORDER BY total_revenue DESC) AS rn -- Rank employees based on total revenue
    FROM 
        RevenueGenerated
)
SELECT 
...
FROM 
    RankedEmployees
WHERE 
    rn <= 3; -- Select only the top three staff members
...
SELECT 
    ...
    (SELECT SUM(pay.amount) 
     ...
     WHERE r.staff_id = s.staff_id -- Filter by staff ID
...
WHERE 
    EXISTS (SELECT 1 
            FROM rental AS r
            JOIN payment AS pay ON pay.rental_id = r.rental_id -- Join to get payments for rentals
            WHERE r.staff_id = s.staff_id 
              AND EXTRACT(YEAR FROM pay.payment_date) = 2017) -- Ensure the staff member has made rentals in 2017
ORDER BY 
